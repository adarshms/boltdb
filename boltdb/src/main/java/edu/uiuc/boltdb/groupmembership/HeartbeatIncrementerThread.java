...
public class HeartbeatIncrementerThread implements Runnable {
	...
	public void run() {
		try {
			MembershipBean entry = GroupMembership.membershipList
					.get(GroupMembership.pid);

			// Dont update the heartbeat if its less than zero which means the
			// node has voluntarily left.
			if (entry.hearbeatLastReceived <= 0)
				return;
